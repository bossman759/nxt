...
<table class="table table-bordered" cellspacing="10px">
  ...
  <thead>
    <tr class="success">
      <th><img src="http://nxt.comxa.com/images/glyphicons_150_edit.png"></img> Sumbit an Article</th>
    </tr>
  </thead>
  <tbody>
    <tr>
	...
	$text = mysql_real_escape_string(stripslashes(strip_tags($_POST['text'])));

	if($title){

		if($text){
		
			$sql = mysql_query("SELECT * FROM Articles WHERE title='$title' AND text='$text'");
			...
			mysql_query("INSERT INTO Articles VALUES('', '$title', '$text', '$date')");				
